...
username.send_keys("abcdef" + str(randomInt))
...
if EnteredValueSize <40:
    ...
    print('It doesnt allow more than 40')
...
browser.find_element_by_xpath(
    "//a[text()='" + EnteredValue + "']//parent::td/../td[text()='Admin']/../td/input").click()
time.sleep(5)
browser.find_element_by_id("btnDelete").click()
browser.find_element_by_id("dialogDeleteBtn").click()
time.sleep(5)

#Search for the same user added now
browser.find_element_by_xpath("//input[@id='searchSystemUser_userName']").send_keys(EnteredValue)
dropdown_usertype=Select(browser.find_element_by_xpath("//select[@id='searchSystemUser_userType']"))
dropdown_usertype.select_by_visible_text("Admin")
browser.find_element_by_xpath("//input[@id='searchSystemUser_employeeName_empName']").send_keys("Fiona Grace")
dropdown_status=Select(browser.find_element_by_xpath("//select[@id='searchSystemUser_status']"))
dropdown_status.select_by_visible_text("Enabled")
time.sleep(2)
browser.find_element_by_xpath("//input[@id='searchBtn']").click()
result_text=browser.find_element_by_xpath("//td[normalize-space()='No Records Found']").text
assert result_text=="No Records Found"
...
dropdown_status=Select(browser.find_element_by_xpath("//select[@id='searchSystemUser_status']"))
dropdown_status.select_by_visible_text("Enabled")
browser.find_element_by_xpath("//input[@id='resetBtn']").click()
time.sleep(3)

usernameele=browser.find_element_by_xpath("//input[@id='searchSystemUser_userName']")
usernamevalue=usernameele.get_attribute("value")
print("username is "+usernamevalue)
print("refrehed!")
browser.close()
